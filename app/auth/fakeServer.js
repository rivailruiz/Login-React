  ...
  */
  init () {
    if (localStorage.users === undefined || !localStorage.encrypted) {
      // Set default user
      let ruiz = 'ruiz'
      let ruizSalt = genSalt(ruiz)
      let ruizPass = hashSync('password', ruizSalt)

      users = {
        [ruiz]: hashSync(ruizPass, salt)
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (userExists && compareSync(password, users[username])) {
        ...
        if (userExists) {
          ...
          error = new Error('Usuário inexistente')
        }
  ...
  register (username, password) {
    return new Promise((resolve, reject) => {
      ...
      if (!this.doesUserExist(username)) {
        ...
        reject(new Error('Nome de usuário já em uso.'))
      }
    })
  },
 /**
